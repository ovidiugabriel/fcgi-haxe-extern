...
Client.getConnection = function(className) {
	var cnx = haxe_remoting_HttpAsyncConnection.urlConnect(Client.URL + "/" + className);
...
Client.call = function(className,methodName,params,onResult) {
	Client.getConnection(className).resolve(className).resolve(methodName).call(params,onResult);
};
Client.main = function() {
	Client.call("Dialog","foo",[js_Boot.__cast(1 , Float),js_Boot.__cast(2 , Float)],Client.display);
};
...
Client.URL = "http://localhost/cgi-bin/Server.exe";
